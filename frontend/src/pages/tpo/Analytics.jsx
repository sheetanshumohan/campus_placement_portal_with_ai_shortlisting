    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-green-100 flex items-center gap-4">
          <div className="p-4 bg-green-50 rounded-xl text-green-600"><TrendingUp size={24} /></div>
          <div><p className="text-sm text-gray-500 font-medium">Avg Package Status</p><p className="text-lg font-bold text-gray-900 truncate">{data.placeholders.averagePackage}</p></div>
        </div>
      </div>
            ...
            <div className="space-y-4">
              {data.placedStudents.map((placed, i) => (
                <div key={i} className="border-b border-gray-50 pb-3 last:border-0 last:pb-0">
                  ...
                  <p className="text-sm text-gray-600">Placed at <span className="text-blue-600 font-medium">{placed.company}</span> as {placed.role}</p>
                </div>
              ))}
            </div>
